...
public class BankDetailService implements IBankDetailService{
    ...
    public BankDetail postBankDetail(BankDetailDto bankDetailDto){
        ...
        if (bankDetailDto.getIban() != null && bankDetailDto.getIban().chars().filter(c -> c == (int)' ').count() < 1 ){
                try {
                    if(repository.findByIban(bankDetailDto.getIban()) == null && validateIBAN(bankDetailDto)){
    ...
    public BankDetailDto deleteByIban(String iban){
        ...
        BankDetail delete = repository.findByIban(ib.toString());
        if (delete == null) {
            throw new ApiException("IBAN DOESN'T EXIST", HttpStatus.NOT_FOUND);
        }
        repository.delete(delete);
        return mapper.bankDetailToBankDetailDto(delete);
    }
}
